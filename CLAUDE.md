...
- **Не запускать dev-сервер для проверки.** Визуальную проверку выполняет пользователь самостоятельно. Достаточно убедиться, что `npm run build` проходит без ошибок.
